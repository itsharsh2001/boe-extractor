...
def Extract(input_file: str) -> tuple[str, pd.DataFrame] | tuple[str, None]:
    ...
    # output_dir = r"C:\Users\Dhruv.jain2\OneDrive - Grant Thornton Advisory Private Limited\Documents\My Work\BOE-PDFtoExcel\Jan_PDFs\New folder"
    ...
    tempdir.cleanup()
    return input_file, newdf


def Extract_all(input_directory: str) -> str:
    ...
    for filename in file_list:
        ...
        if newdf[1] is None:
            ...
            status.append("Data Extraction Failed")
            continue

        status.append("Data Extraction Successful")

        finaldf = pd.concat([finaldf, newdf[1]])

    statusdf = pd.DataFrame({"File": files, "Status": status})

    output_file = format_and_save_df(
        df=finaldf, status_df=statusdf, input_dir=input_directory
    )

    return output_file


def Extract_all_multiprocessing(
    input_directory: str,
) -> str:
    if not os.path.isdir(input_directory):
        raise OSError(f"Directory does not exist: {input_directory}")
    ...
    print(f"Number of Processors: {num_processes}")
    pool = multiprocessing.Pool(processes=num_processes - 1)
    results = pool.map(Extract, file_list)
    pool.close()
    pool.join()
    ...
    for i in results:
        if i[1] is None:
            file_names.append(os.path.basename(i[0]))
            status.append("Data Extraction Failed")
        else:
            finaldf = pd.concat([finaldf, i[1]])
            file_names.append(os.path.basename(i[0]))
            status.append("Data Extraction Successful")

    status_df = pd.DataFrame({"File": file_names, "Status": status})

    output_file = format_and_save_df(
        df=finaldf, status_df=status_df, input_dir=input_directory
    )
...
def format_and_save_df(
    df: pd.DataFrame, status_df: pd.DataFrame, input_dir: str
) -> str:
    tempdir = tempfile.TemporaryDirectory(
        ignore_cleanup_errors=True, prefix="Grant_Thornton"
    )
    ...
    excelfilename = os.path.join(output_dir, "output.xlsx")
    statusfilename = os.path.join(output_dir, "status.xlsx")

    for i in [excelfilename, statusfilename]:
        writer = pd.ExcelWriter(i, engine="xlsxwriter")
        if i == excelfilename:
            df.to_excel(writer, index=False, startrow=2, header=False)
        else:
            status_df.to_excel(writer, index=False, startrow=2, header=False)
    ...
    zipfilename = f"output {str(datetime.now())[:19]}.zip"
    zipfilename = os.path.join(input_dir, zipfilename.replace(":", ";"))
    while os.path.exists(zipfilename):
        zipfilename = f"output {str(datetime.now())[:19]}.zip"
        zipfilename = os.path.join(input_dir, zipfilename.replace(":", ";"))

    with zipfile.ZipFile(zipfilename, "w") as zipf:
        zipf.write(excelfilename, arcname="output.xlsx")
        zipf.write(statusfilename, arcname="status.xlsx")
...
def extract_data_from_excel(input_file: str) -> pd.DataFrame:
    ...
    hawb_mawb_df = pd.read_excel(input_file, sheet_name="Sheet1", skiprows=row, nrows=2)
    # hawb_mawb_df = hawb_mawb_df.dropna(how="all", axis=1)
    a = list(hawb_mawb_df.iloc[0, list(hawb_mawb_df.columns).index("8.HAWB NO") :])

    hawb_no = a[0]
    if hawb_no not in unwanted_values and str(hawb_no) != "nan":
        invoicedf["HAWB No."] = hawb_no
        count = 1
        hawb_date = a[1]

        while str(hawb_date) == "nan" or hawb_date in unwanted_values:
            count += 1
            if count == len(a):
                hawb_date = np.nan
                break
        ...
        invoicedf["HAWB Date"] = hawb_date
    else:
        invoicedf["HAWB No."] = hawb_no
        invoicedf["HAWB Date"] = np.nan

    # MAWB No. & Date
    if "BOND" in str(hawb_mawb_df.iloc[1, 0]):
        ...
        try:
            b = hawb_mawb_df.iloc[
                :, list(hawb_mawb_df.columns).index("6.MAWB NO") :
            ].reset_index(drop=True)
            if str(b.iloc[0, 0]) == "nan":
                invoicedf["MAWB No."] = np.nan
                invoicedf["MAWB Date"] = np.nan
            else:
                invoicedf["MAWB No."] = str(b.iloc[0, 0]) + str(b.iloc[1, 0])
                invoicedf["MAWB Date"] = b.iloc[0, 1]
        except:
            invoicedf["MAWB No."] = np.nan
            invoicedf["MAWB Date"] = np.nan
    ...
    if invoicedf["Total IGST"].sum() == 0:
        try:
            if (
                sum(
                    invoicedf["Total Duty"]
                    - invoicedf["Total BCD"]
                    - invoicedf["Total SWS"]
                )
                > 0
            ):
                invoicedf["Total IGST"] = (
                    invoicedf["Total Duty"]
                    - invoicedf["Total BCD"]
                    - invoicedf["Total SWS"]
                )
...
if __name__ == "__main__":
    # directory = r"C:\Users\Dhruv.jain2\OneDrive - Grant Thornton Advisory Private Limited\Documents\My Work\BOE-PDFtoExcel\BOE_PDFs"
    directory = r"C:\Users\Dhruv.jain2\OneDrive - Grant Thornton Advisory Private Limited\Documents\My Work\BOE-PDFtoExcel\Jan_PDFs"
    print(Extract_all_multiprocessing(input_directory=directory))
